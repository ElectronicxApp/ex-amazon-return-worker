"""
DHL Tracking SQLAlchemy models for the worker.

Tables:
- DHLTrackingData: Shipment-level tracking information (one record per tracking number)
- DHLTrackingEvent: Timeline events (multiple per tracking number)
- DHLTrackingSignature: Proof of delivery signatures
"""
...
class DHLTrackingData(Base):
    """Shipment-level DHL tracking data, one record per tracking number."""
    ...
    dest_country = Column(String(10), nullable=True)
    ...
    __table_args__ = (
        CheckConstraint("tracking_state IN ('active', 'delivered', 'expired', 'exception')", name="ck_tracking_state"),
...
class DHLTrackingEvent(Base):
    ...
    tracking_number = Column(String(100), ForeignKey("dhl_tracking_data.tracking_number", ondelete="CASCADE"), nullable=False, index=True)
    ...
    event_country = Column(String(10), nullable=True)
...
class DHLTrackingSignature(Base):
    """Proof of delivery signature image."""
